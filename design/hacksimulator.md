...
- Two-Pass 실행 모델 (HTTP 병렬 → 브라우저 순차)
...
| 3a | Pass 1: HTTP 테스트 | attack-executor × N | sonnet | **병렬** |
| 3b | Pass 2: 브라우저 테스트 | attack-executor × M | sonnet | 순차 |
| 3c | Pass 3: 익스플로잇 | attack-executor × K | sonnet | 순차 |
...
sequenceDiagram
    ...
    rect rgb(255, 248, 240)
        Note over Skill,ExHTTP: Step 8 Pass 1: HTTP 단계 (병렬)
        par passive
...
| 6 | api | GraphQL, REST API 열거, API 버전 우회 | Pass 1 (HTTP 병렬) |
...
flowchart TD
    ...
    Approve -->|"yes"| Execute["Step 8 진행:<br>Two-Pass 실행"]
    Approve -->|"no"| Reject["선택지 표시:<br>1. 강도 조정<br>2. 타겟 URL 변경<br>3. codeinspector 먼저 실행"]
    Reject --> Stop([중단])
```

## Step 8: Two-Pass 실행 모델

```mermaid
flowchart TD
    Approved([계획 승인]) --> Pass1["Pass 1: HTTP 단계 (병렬)"]

    Pass1 --> P1["phase-1-passive"]
    Pass1 --> P2["phase-2-injection"]
    Pass1 --> P3["phase-3-auth"]
    Pass1 --> P4["phase-4-app-logic"]
    Pass1 --> P5["phase-5-business-logic"]
    Pass1 --> P6["phase-6-api"]
...
| 서브에이전트 | 5개 **병렬** | planner 1개 + executor **N개** (Two-Pass) |
